...
const AppSettingsPage = () => {
  ...
  const keys = !selectedUser ? [] : [...listGetters(selectedUser), ...Object.keys(selectedUser)];
  ...
  function handleClickRole(entityName: string, id: string, roleName: string, action: 'add' | 'del') {
    ...
    <>
      ...
      <p>
        {AVAILABLE_I18N_LANGUAGES.map((newLang, index, newLangArr) => {
          ...
          return (
            <span key={`lang-${index}`}>
              <span className={styles.join(' ')} onClick={() => setLocale(newLang)}>
                {newLang}
              </span>
        ...
        .map((instance: UserEntityRoleMap, instanceIndex: number) => {
          ...
          return (
            <EntityInstanceRole
              ...
              entityInstanceName={
                dataSource?.find((element: EntityModel<EntityTemplate>) => element.id === instance.id)?.name
              }
...
export const UserRoleName = (props: any) => {
  ...
  return (
    <div className="user-role-name">
      <span>{roleName}</span>
      <span className="user-role-delete-button" onClick={() => handleClickRole(entityName, id, roleName, 'del')}>
        {' '}
        X{' '}
      </span>{' '}
    </div>
...
export const FormSelectComponent = (props: any) => {
  ...
  return (
    <FloatingLabel style={{ width: '100%' }} controlId="floatingSelect" label={label}>
      <Form.Select aria-label="Default select example" onChange={handleSelection}>
...
const EntityRolesForm = (props: any) => {
  ...
  return (
    <>
      ...
      <FormSelectComponent label={`Seleccione un ${entityName}`} options={options} />
    </>
  );
};
