...
const buildFormValues = (draft?: CalendarActivityDraft): CalendarActivityFormValues => {
...
const buildActivityPayload = (formValues: CalendarActivityFormValues, image?: string | null): CalendarActivityTemplate => {
...
export const CalendarActivityForm = ({
  open,
  draft,
  fullScreen,
  saveErrorMessage,
  ...
  const formMethods = useForm<CalendarActivityFormValues>({ defaultValues: buildFormValues() });
  ...
  const handlers = {
    ...
    onSubmit: async (formValues: CalendarActivityFormValues) => {
