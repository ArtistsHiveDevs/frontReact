...
const MyApplicationsPage = () => {
  ...
  // Calcular directamente de loggedUser (valores derivados síncronos)
  ...
  const [previousApplication, setPreviousApplication] = useState<OpenCallApplicationModel>(undefined);
  const [submissionSucceeded, setSubmissionSucceeded] = useState(false);
  const [isCheckingPreviousApplications, setIsCheckingPreviousApplications] = useState(true);

  useEffect(() => {
    // Cargar datos del artista si existe el identifier
    if (!!loggedUser?.currentProfileInfo?.identifier) {
  ...
  // Actualizar previousApplication cuando cambien applications o currentArtistId
  useEffect(() => {
    if (currentArtistId) {
      const found = applications.find((app) => app.openCallId === openCallId && app.artistId === currentArtistId);
      setPreviousApplication(found);
    } else {
      setPreviousApplication(undefined);
    }
  }, [applications, currentArtistId, openCallId]);

  // Actualizar submissionSucceeded cuando cambien submitted o createdApplication
  useEffect(() => {
    const belongsToThisOpenCallAndArtist =
      !!createdApplication &&
      createdApplication.openCallId === openCallId &&
      createdApplication.artistId === currentArtistId;
    setSubmissionSucceeded(submitted && belongsToThisOpenCallAndArtist);
  }, [submitted, createdApplication, openCallId, currentArtistId]);

  // Actualizar isCheckingPreviousApplications
  useEffect(() => {
    setIsCheckingPreviousApplications(!submitted && (!previousApplicationsRequested || loading));
  }, [submitted, previousApplicationsRequested, loading]);
  ...
  // Renderizar basándose en estados reactivos
  const renderContent = () => {
    // Esperar a que loggedUser esté cargado antes de verificar el perfil
    if (!loggedUser) {
      return <AppLoader />;
    }
      ...
      return (
        <div className="open-call-page">
          <div className="open-call-header">
            <h1 className="open-call-title">{translate('no_artist_profile.title')} TTTTTT</h1>
