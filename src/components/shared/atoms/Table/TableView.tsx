      ...
      <TableRow>
        ...
        {columns.map((headCell) => (
          ...
          >
            ...
            >
              {translateText([translationBasePath, headCell].join('.'))}
              {orderBy === headCell.id ? (
    ...
    >
      ...
      {numSelected > 0 ? (
        <Tooltip title="Delete">
          <IconButton>X{/* <DeleteIcon /> */}</IconButton>
        </Tooltip>
      ) : (
        <Tooltip title="Filter list">
          <IconButton>F{/* <FilterListIcon /> */}</IconButton>
        </Tooltip>
      )}
    </Toolbar>
  );
...
export const TableView = (props: { config: TableViewConfig }) => {
  ...
  const [rowsPerPage, setRowsPerPage] = React.useState(VISIBLE_ROWS_PER_PAGE);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
          ...
          <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size={dense ? 'small' : 'medium'}>
            ...
            <TableBody>
              {visibleRows.map((row: any, index) => {
                const isItemSelected = false; //isSelected(row.id);
        ...
        {visibleRows.length > VISIBLE_ROWS_PER_PAGE && (
          <TablePagination
